...
\midrule
\texttt{tool}
  & Internal method dispatch: the goroutine receives an unprefixed
    command name (e.g., \texttt{new\_game}, \texttt{move},
    \texttt{look}) --- the JSON-RPC handler strips the \texttt{game.}
    prefix before sending.  Used by passthrough mode for every
    request and by normal mode for \texttt{new\_game}. \\
\texttt{run\_loop}
...
\texttt{session.quit}
  & ---
  & Only meaningful inside an active normal-mode game loop:
    \texttt{RPCRenderer} injects a \texttt{quit} input event,
    the game loop narrates a farewell, and the final response is
    \texttt{PlayResponse} with \field{quit}=\texttt{true}. A bare
    \texttt{session.quit} outside the play loop is ignored. \\
\texttt{game.new}
