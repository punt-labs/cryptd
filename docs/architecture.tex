...
\cmd{cryptd} server
  ...
  & JSON-RPC 2.0 daemon; exposes 15 MCP tools over Unix socket or TCP.
    Normal mode (SLM narrator) and passthrough mode (raw MCP). \\
\cmd{crypt} client
  & Go binary
...
Claude Code session (DM)
    `-- mcp-proxy  <10ms, <10MB
            ...
            | NDJSON Unix socket
            v
    crypt serve   <-- shared daemon, one per adventure
...
\textbf{1 — Engine Core}
  & Go engine (\texttt{GameState}, \texttt{Character},
    ...
    \cmd{crypt serve}, \cmd{crypt solo}, \cmd{crypt headless};
...
Acceptance     & Release      & $<$10~min & YAML game scripts through \cmd{crypt headless}; full adventures \\
\bottomrule
\end{tabular}
\end{center}
...
Requires \cmd{cryptd} binary pre-built.  Asserts observable
