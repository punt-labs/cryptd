...
\midrule
\textbf{Invoked by}
  & \cmd{crypt} client $\to$ \cmd{cryptd serve}
  & \cmd{crypt} client $\to$ \cmd{cryptd serve}
  & \cmd{cryptd serve -t} \\
\bottomrule
\end{tabular}
...
Each tier provides both a \iface{CommandInterpreter} and a
\iface{Narrator} implementation.  The chain is:

\begin{enumerate}
  \item \textbf{Large (Claude API)}: \texttt{LLMInterpreter} +
    \texttt{LLMNarrator} via OpenAI-compatible
    \texttt{/v1/chat/completions} at \texttt{api.anthropic.com}.
    Activated by \texttt{CRYPTD\_API\_KEY} env var or \cmd{--api-key}
    flag.  A startup probe validates the key before committing.
...
\cmd{cryptd} server
  & Go binary
  & JSON-RPC 2.0 daemon; exposes 15 MCP tools over Unix socket or TCP.
    ...
    connections; game registry tracks game goroutines.  Normal mode
    (interpreter + narrator + RPCRenderer) and passthrough mode (raw
    MCP tool dispatch). \\
LLM inference
  & Go (\pkg{internal/inference})
  & OpenAI-compatible HTTP client for \texttt{/v1/chat/completions}.
    Used by \texttt{LLMInterpreter}, \texttt{LLMNarrator},
    \texttt{SLMInterpreter}, and \texttt{SLMNarrator}.  Supports
...
\begin{lstlisting}[title={Passthrough mode (Claude Code plugin -> cryptd serve --passthrough)}]
Claude Code session
    |
    | NDJSON over Unix socket or TCP
    v
cryptd serve --passthrough
    |-- Session/Game registries (same as normal)
    |-- tools/call dispatched directly to engine
    `-- Structured JSON result returned (no interpretation/narration)
\end{lstlisting}

Passthrough mode: Claude Code calls MCP tools directly.  No
interpreter or narrator is involved.  Each \texttt{tools/call}
request is routed through the game goroutine's command channel for
serialized state access.  The LLM (Claude) acts as its own
...
\begin{enumerate}
  \item \textbf{Connection.}
    Client connects and sends \texttt{initialize} with an optional
    \field{session\_id}.  Server returns \field{session\_id} and
    \field{has\_game} (true if this session already has an active
    game from a previous connection).
  \item \textbf{Game start or resume.}
    If \field{has\_game} is false, client sends \texttt{tools/call}
    with \tool{new\_game}.  Server creates a game goroutine, sends
    the initial room, then enters the game loop via
    \texttt{Game.RunLoop()}.  If \field{has\_game} is true, the
    server enters \texttt{resumeGameLoop} immediately --- no
    \tool{new\_game} needed.  The current room is re-rendered.
  \item \textbf{Play loop.}
    Client sends \texttt{play} requests with free text.  Inside the
    game goroutine: \texttt{RPCRenderer.Events()} reads the text,
    the interpreter converts it to an \texttt{EngineAction}, the
    engine executes it, the narrator generates prose, and
    \texttt{RPCRenderer.Render()} writes a \texttt{PlayResponse}
    back to the client.
...
Passthrough mode has no game loop.  Each \texttt{tools/call} request
is dispatched to the game goroutine via \texttt{Game.Send()}.  The
game goroutine executes the tool, returns structured JSON, and the
handler writes it back as a \texttt{ToolResult}.  Claude Code acts
as its own interpreter and narrator.
...
\midrule
\texttt{tool}
  & Passthrough dispatch: execute a named MCP tool against the engine
    and return the result.  Used by passthrough mode for every request
    and by normal mode for \tool{new\_game}. \\
\texttt{run\_loop}
...
\texttt{LLMInterpreter}
  & \mode{dm} (Large tier)
  & Claude API via OpenAI-compatible \texttt{/v1/chat/completions};
...
\texttt{SLMInterpreter}
  ...
  & ollama or any OpenAI-compatible server; uses
    \pkg{internal/inference} client; same rules-first fallback. \\
\texttt{RulesInterpreter}
...
\texttt{TemplateNarrator}
  ...
  & Go text/template: ``You move north. You are in the \{room.name\}.''
    Also serves as fallback for both LLM and SLM narrators on
    inference failure. \\
\bottomrule
\end{tabular}
\end{center}
...
\texttt{RPCRenderer}
  ...
  & Bridges the game loop to the network connection in normal mode.
    \texttt{Render()} serializes a \texttt{PlayResponse} as NDJSON.
...
The engine exposes game actions as MCP tools over JSON-RPC 2.0.
The protocol methods are \texttt{initialize}, \texttt{tools/list},
\texttt{tools/call}, and \texttt{play} (normal mode text input).
...
\texttt{tools/call}
  ...
  & \texttt{ToolResult} (content array + isError) \\
\texttt{play}
  & \field{text}
  & \texttt{PlayResponse}: \field{text}, \field{state?},
    \field{dead?}, \field{quit?} \\
\bottomrule
\end{tabular}
\end{center}

\subsection{Session Management Tools}
...
\tool{new\_game}
  & \field{scenario\_id}, \field{character\_name},
    \field{character\_class}
  & Room description, exits, items, hero summary \\
\tool{save\_game}
  & \field{slot?}
  & Slot name and save file path \\
\tool{load\_game}
  & \field{slot?}
  & Room description, exits, items, hero summary \\
\bottomrule
...
\textbf{Tool} & \textbf{Arguments} & \textbf{Returns} \\
\midrule
\tool{pick\_up}   & \field{item\_id}  & Item details, hero summary \\
\tool{drop}       & \field{item\_id}  & Item details, hero summary \\
\tool{equip}      & \field{item\_id}  & Item, slot, hero summary \\
\tool{unequip}    & \field{slot}      & Item, slot, hero summary \\
\tool{examine}    & \field{item\_id}  & Item details \\
\tool{inventory}  & ---               & Items, equipped, weight, capacity \\
\bottomrule
\end{tabular}
\end{center}
...
\texttt{/v1/chat/completions} wire format via
\pkg{internal/inference.Client}.  The same client code works for
Claude API, ollama, llama.cpp, and any other compatible server.
...
\midrule
\textbf{Large}
  & Claude API (\texttt{api.anthropic.com})
  & Activated by \texttt{CRYPTD\_API\_KEY} env var or \cmd{--api-key}
    flag.  Default model: \texttt{claude-sonnet-4-20250514} (configurable via
    \cmd{--model}).  Uses \texttt{Authorization: Bearer <key>} header.
    A startup probe validates the key with a 5~s timeout before
    committing to this tier.  On probe failure, falls through to
    Medium. \\
\textbf{Medium/Small}
  & ollama or any OpenAI-compatible server
  & Auto-detected by probing default endpoints
    (\texttt{localhost:11434}).  Preferred models: \texttt{gemma3:1b},
    \texttt{llama3.2:3b}.  Configurable via \cmd{--model}. \\
\textbf{Tiny}
  & Rules + Templates (no inference)
  & Deterministic fallback when no inference server is available.
    \texttt{RulesInterpreter} + \texttt{TemplateNarrator}.
...
\textbf{M8 — Server Thin Slice}
  & \cmd{cryptd serve} daemon with JSON-RPC 2.0 over Unix socket
    ...
    15 MCP tools; daemonization; PID file management.
  & Complete \\
\midrule
\textbf{M9 — LLM Integration}
  & Three-tier inference chain (Claude API $\to$ ollama SLM $\to$
...
\textbf{M10 — Concurrent Sessions}
  & Game-as-goroutine; session registry; game registry; session
    resume via \field{session\_id} in initialize; concurrent
...
\begin{enumerate}[label=\textbf{Q\arabic*}.]
  ...
  \item \textbf{Go MCP SDK.}
    ...
    \pkg{internal/daemon} and \pkg{internal/protocol}.  Approximately
    500~lines; no external dependency.
